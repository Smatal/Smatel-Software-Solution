        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {servicesOverview.services.map((service, index) => {
              ...
              return (
              <Link key={index} href={service.href} className="group relative h-96 w-full overflow-hidden rounded-3xl shadow-lg transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 border border-slate-200/50 bg-white">
                ...
                <div className="absolute inset-0 p-8 flex flex-col justify-end z-20">
                  ...
                  <div className="flex items-center text-sm font-semibold text-indigo-600 group-hover:text-indigo-800 transition-colors">
                    View Service Details <ArrowRight className="ml-2 w-4 h-4 transition-transform group-hover:translate-x-2" />
                  </div>
                </div>
              </Link>
              );
